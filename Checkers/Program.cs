...
public class Checkers
{
    ...
    public List<Tuple<int,int>> GenMovPiece(int i,int j)
    {
        ...
        if (IsValidePosition(i, j)){
            ...
            {
                case Piece.NN:
                    Console.WriteLine("The box is empty .Please re_enter different coordinates");
    ...
    static void Main(String[] args)
    {
        ...
        while (checkers.NumBlackPiece() > 0 && checkers.NumWhitePiece() > 0)
        {
            ...
            var captures = checkers.GenCapturePlayers(turn);
            var moves = checkers.GenMovPlayers(turn);

            if (captures.Count == 0 && moves.Count == 0)
            {
            ...
            Console.Write("From Row (0-7): ");
            int fromI = int.Parse(Console.ReadLine());

            Console.Write("From Col (0-7): ");
            int fromJ = int.Parse(Console.ReadLine());

            Console.Write("To Row (0-7): ");
            int toI = int.Parse(Console.ReadLine());

            Console.Write("To Col (0-7): ");
            int toJ = int.Parse(Console.ReadLine());

            
            var possibleMoves = checkers.GenMovPiece(fromI, fromJ);
            var possibleCaptures = checkers.GenCapturePiece(fromI, fromJ);

            var move = Tuple.Create(toI, toJ);

            if (possibleCaptures.Contains(move))
            {
            ...
            else if (possibleMoves.Contains(move) && captures.Count == 0)
            {
